...
const DEFAULT_GLOW_COLOR = '13, 148, 136' // Use AlignAV's accent green (RGB 13, 148, 136) for native theme matching
...
export const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const container = gridRef?.current || document.body
      const section = gridRef?.current ? gridRef.current.closest('.bento-section') : document.body
    ...
    const handleMouseLeave = () => {
      ...
      const container = gridRef?.current || document.body
      container.querySelectorAll('.magic-bento-card').forEach(card => {
        (card as HTMLElement).style.setProperty('--glow-intensity', '0')
      })
